...
use crate::db::{
    self, comment_refresh_cutoff, recent_cutoff, Database, HydrationMode, HydrationPolicy,
    HydrationResource, HydrationStatus, IndexUpsertOutcome, IssueIndexEntry,
};
...
impl LinearClient {
    ...
    /// Compatibility wrapper preserving the original explicit-refresh behavior.
    pub async fn hydrate_issue(
        ...
        self.hydrate_issue_with_mode(
            db,
            issue_id,
            workspace_id,
            HydrationMode::ForceRefresh,
            progress,
        )
        .await
    }

    /// Hydrate one selected issue according to an explicit refresh mode.
    pub async fn hydrate_issue_with_mode(
        ...
        db.ensure_hydration_state_for_issue(workspace_id, &issue, "selected")?;
        match mode {
            HydrationMode::IfNeeded => {
                db.queue_stale_issue_comment_hydration(
                    workspace_id,
                    &issue.id,
                    &comment_refresh_cutoff(Utc::now()),
                )?;
            }
            HydrationMode::ForceRefresh => {
                db.requeue_issue_hydration(workspace_id, &issue.id, "explicit_force")?;
            }
        }
    ...
    async fn hydrate_one(
        ...
        for resource_state in initial.resources {
            let should_attempt = match resource_state.status {
                HydrationStatus::Pending => true,
                HydrationStatus::Retryable => match resource_state.next_retry_at.as_deref() {
                    None => true,
                    Some(value) => DateTime::parse_from_rfc3339(value)
                        .is_ok_and(|retry_at| retry_at.with_timezone(&Utc) <= now),
                },
                _ => false,
            };
            if !should_attempt {
