...
#[derive(Clone, Copy, Debug, PartialEq, Eq, uniffi::Enum)]
pub enum RtHydrationMode {
    IfNeeded,
    ForceRefresh,
}

impl From<RtHydrationMode> for crate::db::HydrationMode {
    fn from(value: RtHydrationMode) -> Self {
        match value {
            RtHydrationMode::IfNeeded => Self::IfNeeded,
            RtHydrationMode::ForceRefresh => Self::ForceRefresh,
        }
    }
}
...
impl From<crate::linear::HydrationBatchResult> for RtHydrationBatchResult {
    fn from(value: crate::linear::HydrationBatchResult) -> Self {
        Self {
            ...
            retryable_failures: value.retryable_failures as u64,
            permanent_failures: value.permanent_failures as u64,
            required_failures: value.required_failures as u64,
            comment_failures: value.comment_failures as u64,
            rate_limited: value.rate_limited,
        }
    }
}
...
impl RectilinearEngine {
    ...
    /// Synchronize and reconcile project metadata and milestones for one team.
    pub async fn sync_team_projects(
        ...
        let client = self.linear_client(&workspace_id).await?;
        let result = client
            .sync_team_projects(&self.db, &team_key, &workspace_id)
            .await
            .map_err(api_error)?;
        Ok(RtProjectSyncResult {
            projects: result.projects as u64,
            milestones: result.milestones as u64,
        })
    }
    ...
    /// Compatibility API: force-refresh all rich resource families.
    ...
    /// Hydrate a selected issue only when needed, or explicitly force-refresh it.
    pub async fn hydrate_issue_with_mode(
        &self,
        issue_id: String,
        workspace_id: String,
        mode: RtHydrationMode,
    ) -> Result<RtIssueHydrationResult, RectilinearError> {
        let client = self.linear_client(&workspace_id).await?;
        let _progress_reset = SyncProgressReset(&self.sync_progress);
        let progress_state = &self.sync_progress;
    ...
    pub async fn embed_issues(
        ...
        let issues = self.db.get_issues_needing_embedding_for_model(
            team.as_deref(),
            false,
            &workspace_id,
            Some(&model_name),
        ...
        // Pre-chunk issues whose model or exact source content is stale.
        struct IssueChunks {
            issue_id: String,
        ...
        for issue in to_process {
            ...
            pending.push(IssueChunks {
                ...
                source_content_hash: crate::embedding::issue_content_hash(
                    &issue.title,
        ...
        let result: Result<u64, RectilinearError> = async {
            ...
            for ic in &pending {
                ...
                self.db.upsert_chunks_with_model_and_hash(
                    &ic.issue_id,
                    &chunk_data,
                    &model_name,
